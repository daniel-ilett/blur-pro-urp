Shader "Custom/BotWGrass"
{
	Properties
	{
		...
		_GrassColorVariation("Grass Color Variation", Range(0, 1)) = 0.1
	}
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			struct GeomData
			{
				...
				nointerpolation float3 rootPos : TEXCOORD2;
			};
			...
			GeomData TransformGeomToClip(float3 pos, float3 offset, float3x3 transformationMatrix, float2 uv, float3 rootPos)
			{
				GeomData o;
			...
			void geom(point VertexOutput input[1], inout TriangleStream<GeomData> triStream)
			{
				...
				if (grassVisibility >= _GrassThreshold)
				{
					...
					for (int i = 0; i < BLADE_SEGMENTS; ++i)
					{
						...
						triStream.Append(TransformGeomToClip(pos, float3( offset.x, offset.y, offset.z), transformationMatrix, float2(0, t), rootPos));
						triStream.Append(TransformGeomToClip(pos, float3(-offset.x, offset.y, offset.z), transformationMatrix, float2(1, t), rootPos));
					}

					// Add the final vertex at the tip of the grass blade.
					triStream.Append(TransformGeomToClip(pos, float3(0, forward, height), tipTransformationMatrix, float2(0.5, 1), rootPos));
        ...
        Pass
        {
            ...
            float4 frag (GeomData i) : SV_Target
            {
				...
				float4 tipColor = _TipColor - rand(i.rootPos) * _GrassColorVariation;
